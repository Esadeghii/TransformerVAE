...
class SequenceModel(Model):
    ...
    def compute(self,transformer_encoder,x,mask,src_key_padding_mask,i_layer,d_model,num_heads):
        ...
        attn_probs = F.softmax(v*attn_probs, dim=-1)
        return attn_logits,attn_probs

    def extract_maps(self,x,mask,src_key_padding_mask):
        attn_logits_maps = []
        attn_probs_maps = []
        ...
        with torch.no_grad():
            for i in range(num_layers):
                ...
                attn_logits,attn_probs = self.compute(self.transformer_encoder,h,mask,src_key_padding_mask,i,d_model,num_heads)
                attn_logits_maps.append(attn_logits) 
