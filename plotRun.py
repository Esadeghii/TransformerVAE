...
def genFigure(filePath):
    ...
    fig, ax = plt.subplots(7,2,sharex=False,sharey=False,)
    ...
    ax[1,1].remove()
    ax[1,0].plot(epochsT, rLossV, label="r Loss")
    ax[1,0].plot(epochsT, kdLossV, label="kld Loss")
    ax[1,0].plot(epochsT, regLossV, label="reg Loss")
    ax[1,0].plot(epochsV, lossV, label="Valid Loss")
    ...
    ax[2,0].plot(epochsT, accT, label="Train Accuracy")
    ax[2,0].plot(epochsV, accV, label="Valid Accuracy")
    ax[2,0].legend(loc="center left", bbox_to_anchor=(1, 0.5))
    ax[2,0].set(xlabel="epoch",ylabel="Accuracy")
    ax[2,0].text(x=1300, y=0.62, s=f"Training Accuracy: {'{:.3f}'.format(accT[len(accT) - 1])}")
    ax[2,0].text(x=1300, y=0.50, s=f"Testing Accuracy: {'{:.3f}'.format(accV[len(accV) - 1])}")
    # ax[1,0].label_outer()


    for i in range(int(latentDims)):
        ax[3,0].plot(dimRange, wldimCors[:,i], label=str(i), ls="-" if i == 0 else "--")
        ax[4,0].plot(dimRange, lidimCors[:,i], label=str(i), ls="-" if i == 1 else "--")
    #ax[2,0].legend(loc="center left", bbox_to_anchor=(1, 0.5), ncol=2)
    ax[3,0].set(xlabel="epoch",ylabel="WL Cor")
    #ax[2,0].label_outer()
    #ax[3,0].legend(loc="center left", bbox_to_anchor=(1, 0.3), ncol=2)
    ax[4,0].set(xlabel="epoch",ylabel="LII Cor")
    #ax[3,0].label_outer()

    for i in range(int(latentDims)):
        ax[3,1].plot(dimRangev, wldimCorsv[:,i], label=str(i), ls="-" if i == 0 else "--")
        ax[4,1].plot(dimRangev, lidimCorsv[:,i], label=str(i), ls="-" if i == 1 else "--")
    ax[3,1].legend(loc="center left", bbox_to_anchor=(1, 0.5), ncol=2)
    ax[3,1].set(xlabel="epoch",ylabel="WL Cor val")
    #ax[2,1].label_outer()
    ax[4,1].legend(loc="center left", bbox_to_anchor=(1, 0.3), ncol=2)
    ax[4,1].set(xlabel="epoch",ylabel="LII Cor val")
    #ax[3,1].label_outer()
    ...
    ax[5,0].plot(y3 ,'g', label=help1[2])
    #ax[4,0].plot(y4 ,'darkgreen', label=help1[3])
    ax[5,0].plot(y5 ,'coral', label=help1[4])
    #ax[4,0].plot(y6 ,'tomato', label=help1[5])
    ax[5,0].plot(y7 ,'tab:red', label=help1[6])
    #ax[4,0].plot(y8 ,'brown', label=help1[7])
    ax[5,0].plot(y9 ,'k', label=help1[8])
    #ax[4,0].plot(y10,'gray', label=help1[9])
    ax[5,0].set(xlabel="epoch",ylabel='WL avg')#+str(round(np.mean(wavg),4)))
    ...
    ax[5,1].plot(yv3 ,'g', label=help1[2])
    #ax[8].plot(yv4 ,'darkgreen', label=help1[3])
    ax[5,1].plot(yv5 ,'coral', label=help1[4])
    #ax[8].plot(yv6 ,'tomato', label=help1[5])
    ax[5,1].plot(yv7 ,'tab:red', label=help1[6])
    #ax[8].plot(yv8 ,'brown', label=help1[7])
    ax[5,1].plot(yv9 ,'k', label=help1[8])
    #ax[8].plot(yv10,'gray', label=help1[9])
    ax[5,1].set(xlabel="epoch",ylabel='WL avg val')#+str(round(np.mean(wavgv),4)))
    ax[5,1].legend(loc="center left", bbox_to_anchor=(1, 0.7), ncol=1)
    ...
    #ax[4,0].plot(y10,'gray', label=help1[9])
    ax[6,0].set(xlabel="epoch",ylabel='LII avg')#+str(round(np.mean(liiavg),4)))
    #ax[4,0].legend(loc="center left", bbox_to_anchor=(1, 0.7), ncol=2)

    #ax[4,0].label_outer()

    help1= { 0:'New Mean', 1:'New Std', 2:'Very dark', 3:'Green Std', 4:'Dark', 5:'Red Std', 6:'Bright', 7:'Very Red Std', 8:'Very Bright', 9:'Near-IR Std'}
    ...
    #ax[8].plot(yv10,'gray', label=help1[9])
    ax[6,1].set(xlabel="epoch",ylabel='LII avg val')#+str(round(np.mean(liiavgv),4)))
    ax[6,1].legend(loc="center left", bbox_to_anchor=(1, 0.7), ncol=1)
